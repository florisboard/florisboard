...
class TextKeyboardCache(ioDispatcher: CoroutineDispatcher = Dispatchers.IO) {
     ...
     */
    fun clear() {
        flogDebug(bha.keys.smartype.lib.devtools.LogTopic.TEXT_KEYBOARD_VIEW) { "Clear whole cache" }
        for (mode in KeyboardMode.entries) {
            cache[mode]!!.clear()
        }
    }
    ...
    fun clear(mode: KeyboardMode) {
        flogDebug(bha.keys.smartype.lib.devtools.LogTopic.TEXT_KEYBOARD_VIEW) { "Clear cache for mode '$mode'" }
        cache[mode]!!.clear()
    }
    ...
    fun clear(subtype: Subtype) {
        flogDebug(bha.keys.smartype.lib.devtools.LogTopic.TEXT_KEYBOARD_VIEW) { "Clear cache for subtype '${subtype.toShortString()}'" }
        for (mode in KeyboardMode.entries) {
    ...
    fun clear(mode: KeyboardMode, subtype: Subtype) {
        flogDebug(bha.keys.smartype.lib.devtools.LogTopic.TEXT_KEYBOARD_VIEW) { "Clear cache for mode '$mode' and subtype '${subtype.toShortString()}'" }
        cache[mode]!!.remove(subtype.hashCode())
    }
    ...
    fun getAsync(mode: KeyboardMode, subtype: Subtype): TextKeyboard? {
        return cache[mode]!![subtype.hashCode()].also {
            flogDebug(bha.keys.smartype.lib.devtools.LogTopic.TEXT_KEYBOARD_VIEW) { "Get keyboard '$mode ${subtype.toShortString()}'" }
        }
    }
    ...
    fun set(mode: KeyboardMode, subtype: Subtype, keyboard: TextKeyboard) {
        flogDebug(bha.keys.smartype.lib.devtools.LogTopic.TEXT_KEYBOARD_VIEW) { "Set keyboard '$mode ${subtype.toShortString()}'" }
