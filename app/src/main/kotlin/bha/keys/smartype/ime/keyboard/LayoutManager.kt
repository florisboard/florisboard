...
import bha.keys.smartype.lib.io.loadJsonAsset
...
class LayoutManager(context: Context) {
    ...
    private fun loadLayoutAsync(ltn: LTN?, allowNullLTN: Boolean) = ioScope.runCatchingAsync {
        ...
        layoutCacheGuard.withLock {
            ...
            if (cached != null) {
                flogDebug(bha.keys.smartype.lib.devtools.LogTopic.LAYOUT_MANAGER) { "Using cache for '${ltn.name}'" }
                return@withLock cached
            } else {
                flogDebug(bha.keys.smartype.lib.devtools.LogTopic.LAYOUT_MANAGER) { "Loading '${ltn.name}'" }
    ...
    private fun loadPopupMappingAsync(subtype: Subtype? = null) = ioScope.runCatchingAsync {
        ...
        popupMappingCacheGuard.withLock {
            ...
            if (cached != null) {
                flogDebug(bha.keys.smartype.lib.devtools.LogTopic.LAYOUT_MANAGER) { "Using cache for '$name'" }
                return@withLock cached
            } else {
                flogDebug(bha.keys.smartype.lib.devtools.LogTopic.LAYOUT_MANAGER) { "Loading '$name'" }
    ...
    private suspend fun mergeLayouts(
        ...
        return TextKeyboard(
            ...
            extendedPopupMapping = extendedPopups.await().onFailure {
                flogWarning(bha.keys.smartype.lib.devtools.LogTopic.LAYOUT_MANAGER) { it.toString() }
            }.getOrNull()?.mapping,
