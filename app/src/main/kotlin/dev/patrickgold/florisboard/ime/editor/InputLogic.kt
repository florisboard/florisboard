...
class InputLogic(context: Context) {
    ...
    val cursorCapsMode: InputAttributes.CapsMode
        get() {
            val ic = currentInputConnection() ?: return InputAttributes.CapsMode.NONE
            return InputAttributes.CapsMode.fromFlags(
                ic.getCursorCapsMode(editorInstance.activeInfo().inputAttributes.capsMode.toFlags())
            )
        }
    ...
    init {
        if (BuildConfig.DEBUG) {
            scope.launch {
                editorInstance.activeContent.collect { editorContent ->
    ...
    fun onUpdateSelection(
        ...
        flogDebug { "oldSel=$oldSel, newSel=$newSel, candidates=$candidates" }

        activeSelection = newSel
        editorInstance.update(newSel)
    }
    ...
    fun startInput(editorInfo: FlorisEditorInfo) {
        ...
        ic.requestCursorUpdates(InputConnection.CURSOR_UPDATE_MONITOR or InputConnection.CURSOR_UPDATE_IMMEDIATE)
    }
    ...
    fun startInputView(editorInfo: FlorisEditorInfo) {
        ...
        if (editorInfo.isRichInputEditor) {
            editorInstance.initialize(editorInfo)
        }
    }
    ...
    fun commitText(text: String): Boolean {
        ...
        when {
            editorInstance.activeInfo().isRawInputEditor || activeSelection.isSelectionMode -> {
                ic.finishComposingText()
                ...
                isWordComponent -> {
                    val composingText = editorInstance.activeContent().composingText
    ...
    fun commitCompletion(text: String): Boolean {
        if (text.isEmpty() || editorInstance.activeInfo().isRawInputEditor) return false
        val ic = currentInputConnection() ?: return false
        ic.beginBatchEdit()
            ...
            ItemType.IMAGE, ItemType.VIDEO -> {
                ...
                if (AndroidVersion.ATLEAST_API25_N_MR1) {
                    ...
                    appContext.grantUriPermission(
                        editorInstance.activeInfo().packageName,
                        item.uri,
                        Intent.FLAG_GRANT_READ_URI_PERMISSION,
                    )
                }
                InputConnectionCompat.commitContent(ic, editorInstance.activeInfo().base, inputContentInfo, flags, null)
            }
        }
    }
    ...
    fun deleteBackwards(): Boolean {
        phantomSpace.setInactive()
        return if (editorInstance.activeInfo().isRawInputEditor) {
    ...
    fun getTextBeforeCursor(n: Int): String {
        if (n < 1) return ""
        return editorInstance.activeContent().textBeforeSelection.takeLast(n)
    }
    ...
    fun getTextAfterCursor(n: Int): String {
        if (n < 1) return ""
        return editorInstance.activeContent().textAfterSelection.take(n)
    }
